...
| *utterance* L-14k v2 [^16] | ~17,2 | early stop ~21k upd. (Modyco) |
| *utterance* L-14k **v3** [^21] | ~21,2 | ~9–10 h GPU (Modyco) |
| *utterance* L-14k **v5** SpecAugment [^27] | **~26,1** | ~7,6 h GPU (Modyco) — **dépasse papier L-14k (~24)** ; **meilleur ST local** |
| *utterance* L-14k v6 long [^31] | ~25,1 | early stop (Modyco, 14 juin) — sous run_026 |
...
- Encodeur **L-14k** : v5 SpecAugment [^27] (**~26,1**, **meilleur local**, vocab 1k) ; ablations **v9** en cours/file (`run_036` warmup 10k, `run_037` SpecAugment fort). **L-114k** v5 [^33] (~23,5, OVH) ; `run_038` freq **en file**.
...
2. ~~**Implémenter le warmup LR** (10 000 pas) dans `4_train.py`~~ — **fait** ; ablation `run_036` (warmup 10k) en cours.
3. ~~**Ajouter SpecAugment**~~ — **fait** (v5+) ; ablations `run_037` (fort) et `run_038` (freq) en file.
...
| L-114k | ~25,2 | **~23,5** [^33] (run_028 OVH) ; ~20,2 [^26] (run_019) |
...
| *utterance* L-14k gelé [^12] | **~15,0** | ~1,4 h GPU (OVH) — **48 tok** |
| *utterance* L-114k gelé [^14] | **~15,2** | ~4–6 h GPU (OVH) — **48 tok** |
| *utterance* L-14k + Qwen2.5-3B [^24] | ~13,0 | ~0,4 h GPU éval (Modyco) — sous Phi-2 |
| *utterance* L-114k v2 (128 tok) [^22] | ~5,6 | ~15 h GPU (OVH) — **échec** vs run_013 |
| *utterance* L-14k v3 (128 tok) [^23] | ~5,5 | ~4 h GPU (Modyco) — **échec** ; hyp. ~3× trop longues |
| *utterance* L-14k replicate (48 tok) [^25] | **~14,2** | ~2,5 h GPU (Modyco) — proche run_012 (15,03) |
...
[^21]: `run_020_transformer_baseline_utterance_large_14k_v3` — 22,05 / **~21,22** test (Modyco, eval dev complet, patience 4)
[^22]: `run_017_speechllm_b1_utterance_large_114k_v2` — 6,56 / **5,60** test (OVH, max 128 tok — échec vs run_013)
[^23]: `run_021_speechllm_b1_utterance_large_14k_v3` — 5,84 / **5,48** test (Modyco, max 128 tok — hyp. ~3× trop longues)
[^24]: `run_018_speechllm_b2bis_utterance_large_14k_qwen25_3b` — 13,96 / **12,95** test (Modyco, Qwen2.5-3B)
[^25]: `run_023_speechllm_b1_utterance_large_14k_replicate` — 15,26 / **14,23** test (Modyco, 48 tok — proche run_012)
[^26]: `run_019_transformer_baseline_utterance_large_114k_v3` — 21,09 / **20,19** test (OVH, eval dev complet)
[^27]: `run_026_transformer_baseline_utterance_large_14k_v5` — 26,57 / **26,12** test (Modyco, SpecAugment, ~7,6 h GPU, early stop @~55k)
[^28]: `run_024_transformer_baseline_utterance_large_14k_v4` — batch 64 ; **0,20 / 0,35** test (collapse, Modyco)
[^29]: `run_022_speechllm_b1_utterance_large_114k_v3` — 5,28 / **4,78** test (OVH, 128 tok — échec)
[^30]: `run_025_transformer_baseline_utterance_large_114k_v4` — batch 64 ; **0,24 / 0,31** test (collapse, OVH)
[^31]: `run_027_transformer_baseline_utterance_large_14k_v6_long` — 26,37 / **25,12** test (Modyco, 120k updates, early stop — sous run_026)
