#!/usr/bin/env bash
# Run 031 — baseline ST utterance L-14k v7 (SPM 5k + SpecAugment, piste 3 Table 8).
#
# Recette run_026 v5 + vocab SentencePiece 5000 (LeBenchmark ~5k).
# Durée estimée : ~8–10 h GPU (budget Modyco).
#
# Lancement nohup (Modyco) :
#   cd ~/S3T && source .venv/bin/activate
#   mkdir -p logs
#   nohup bash 1_Transformer/scripts/run_031_baseline_utterance_large_14k_v7_spm5k_nohup.sh \
#     > logs/run_031_st_14k_v7_wrapper.log 2>&1 &
#   tail -f logs/run_031_transformer_baseline_utterance_large_14k_v7_spm5k_spm_train_eval.log
...
CFG="1_Transformer/configs/fr-en/base_utterance_large_14k_v7_spm5k.yaml"
RUN="run_031_transformer_baseline_utterance_large_14k_v7_spm5k"
...
{
  if [[ ! -f "$SPM_MODEL" ]]; then
    echo "=== $(date -Is) SPM utterance (vocab 5k) — absent, entraînement SPM ==="
...
"
  ...
  echo "=== $(date -Is) TRAIN ${RUN} (L-14k v7: SPM 5k + SpecAugment) ==="
