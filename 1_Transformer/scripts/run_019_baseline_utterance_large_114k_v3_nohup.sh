#!/usr/bin/env bash
# Run 019 — baseline ST utterance L-114k v3 (retry run_016 @ 19,63 test).
#
# Changements : early_stopping_patience 4, eval dev complet (max_eval_batches null).
# Durée estimée : ~9–10 h GPU.
#
# Lancement nohup (OVH) :
...
#
# Lancement nohup (Modyco — nécessite L-114k en cache HF) :
#   cd ~/S3T && source .venv/bin/activate
#   mkdir -p logs
#   nohup bash 1_Transformer/scripts/run_019_baseline_utterance_large_114k_v3_nohup.sh \
#     > logs/run_019_st_114k_v3_wrapper.log 2>&1 &
#   tail -f logs/run_019_transformer_baseline_utterance_large_114k_v3_spm_train_eval.log
...
CFG="1_Transformer/configs/fr-en/base_utterance_large_114k_v3.yaml"
RUN="run_019_transformer_baseline_utterance_large_114k_v3"
...
"
  ...
  echo "=== $(date -Is) TRAIN ${RUN} (L-114k v3: patience 4, eval dev complet) ==="
