#!/usr/bin/env bash
# Run 024 — baseline ST utterance L-14k v4 (batch effectif 64, piste 1 Table 8).
#
# Changements vs v3 (run_020) : grad_accum 64, LR 2e-4, warmup 1k, 10k updates
# (même volume de micro-batches que run_020 : 80k×8 = 10k×64).
# Durée estimée : ~9–10 h GPU (budget Modyco).
#
# Lancement nohup (Modyco) :
#   cd ~/S3T && source .venv/bin/activate
#   mkdir -p logs
#   nohup bash 1_Transformer/scripts/run_024_baseline_utterance_large_14k_v4_nohup.sh \
#     > logs/run_024_st_14k_v4_wrapper.log 2>&1 &
...
CFG="1_Transformer/configs/fr-en/base_utterance_large_14k_v4.yaml"
...
"
  ...
  echo "=== $(date -Is) TRAIN ${RUN} (L-14k v4: batch eff. 64, LR 2e-4) ==="
