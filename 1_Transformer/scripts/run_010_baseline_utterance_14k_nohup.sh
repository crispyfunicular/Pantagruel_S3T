#!/usr/bin/env bash
# Baseline ST Table 8 — utterance, encodeur Pantagruel-L-14k (speech-large-14K).
#
# Prérequis : manifests utterance + SPM fr-en_1000 (voir run_002 ou SPM seul ci-dessous).
# Durée estimée : ~12–18 h GPU (encodeur Large, batch 1).
#
#   nohup bash 1_Transformer/scripts/run_010_baseline_utterance_14k_nohup.sh \
#     > logs/run_010_transformer_utterance_14k_wrapper.log 2>&1 &
...
CFG="1_Transformer/configs/fr-en/base_utterance_large_14k.yaml"
RUN="run_010_transformer_baseline_utterance_large_14k"
...
"
  ...
  echo "=== $(date -Is) TRAIN ${RUN} (Pantagruel-L-14k) ==="
