...
Statut : bench utterance partiel ; ST B-1k run_002 échoué (3,79) ; run_004 v2 **terminé** (16,84 / 16,68) ; ST L-14k `run_010` **échec** (0,00) — **`run_014` v2 en cours** (Modyco) ; speechLLM `run_003` **terminé** (10,00 / 7,47) ; speechLLM L-14k **`run_012` terminé** (15,49 / 15,03, OVH) ; L-114k **`run_013` en cours** (OVH) ; Gemini 3.5 **`run_003_gemini_35_flash_utterance` terminé** (11,66 / 13,39 — recul vs 2.5) ; **`run_003_gemini_35_flash_sentence_like` en cours**
...
Les expériences couvrent deux segmentations : **`sentence_like`** (fusion de segments, runs historiques) et **`utterance`** (segments m-TEDx natifs, bench Pantagruel). Sur **utterance**, la cascade Whisper→Marian atteint **38,17 / 37,41** BLEU (dev/test), devant Gemini **2.5** Flash (**33,76 / 33,72**). Gemini **3.5** Flash (`run_003_gemini_35_flash_utterance`, juin 2026) n’atteint que **11,66 / 13,39** sous le même protocole v1 — hypothèses tronquées (budget `max_output_tokens=256` + thinking) ; comparaison **non concluante** sans relance. La réplication ST Table 8 B-1k est **partielle** : run_002 en échec (**3,79** test) ; run_004 v2 **terminé** (**16,84 / 16,68**). L’encodeur **L-14k** ST (`run_010`) a **échoué** (collapse) ; **`run_014` v2 en cours** (Modyco). speechLLM B-1k (`run_003`) : **10,00 / 7,47** ; speechLLM **L-14k** (`run_012`, OVH) : **15,49 / 15,03** ; L-114k **`run_013` en cours**. Sur **sentence_like**, Gemini 2.5 reste en tête (**21,44 / 23,15**), puis speechLLM dégelé (**19,25 / 18,83**), ST greedy (**16,12 / 14,97**). Les tableaux détaillés sont en **§5** ; ne pas mélanger utterance et sentence_like.
...
| **3.5 Flash** | `gemini-3.5-flash` | `run_003_gemini_35_flash_utterance` — **11,66 / 13,39** (utt., ok) ; `run_003_gemini_35_flash_sentence_like` — **en cours** | `gemini_flash_35_*.yaml` |

**Protocole v1 :** même prompt, `temperature=0`, `max_output_tokens=256`, SacreBLEU signé — **ne pas** réécrire les artefacts 2.5.

**Résultat utterance 3.5 (juin 2026) :** BLEU test **13,39** vs **33,72** (2.5) ; durée **99 min** ; coût API **0,60 $** (`eval/metrics.json`). Hypothèses **~2× plus courtes** que la référence (~9 tokens visibles / segment en moyenne) ; **~75 %** des segments sans fin de phrase plausible → **troncature** probable (budget `max_output_tokens=256` + tokens *thinking* du 3.5). **Interprétation :** comparaison 2.5 vs 3.5 **non concluante** tant qu’un run 3.5 n’a pas été relancé avec budget sortie plus large et thinking minimal.

**Prochaine étape :** relance 3.5 (`max_output_tokens` ↑, `thinking_level: minimal`) ; terminer `sentence_like` en cours.
...
- **3.5 (évalué, incomplet)** : `gemini-3.5-flash` — **11,66 / 13,39** (`utterance`, juin 2026) ; `sentence_like` **en cours**. Écart vs 2.5 surtout dû à des **traductions tronquées** sous `max_output_tokens=256` (thinking 3.5) — relance prévue.
...
| Gemini 3.5 Flash | `run_003_gemini_35_flash_utterance` | `gemini-3.5-flash` | temp 0, max 256 tok | 11,66 | 13,39 | ok (local, 99 min, 0,60 $) |
| Gemini 3.5 Flash | `run_003_gemini_35_flash_sentence_like` | `gemini-3.5-flash` | idem | — | — | **en cours** |
...
| speechLLM B1 **L-114k** | `run_013_speechllm_b1_utterance_large_114k` | `speech-large-114K` gelé → Phi-2 gelé | beam 1, max 48 tok | — | — | **en cours** (OVH) |
...
| ST E2E Transformer **L-14k v2** | `run_014_transformer_baseline_utterance_large_14k_v2` | idem + gel 5k + early stop + LR 1e-4 | greedy (v1) | — | — | **en cours** (Modyco) |
...
| Gemini 3.5 Flash | **13,39** (run préliminaire) | — (en cours) | — |
...
| `run_003_gemini_35_flash_sentence_like` | gemini | sentence_like | gemini-3.5-flash | aucun | temp 0 | — (en cours) |
...
| `run_013_speechllm_b1_utterance_large_114k` | speechllm | utterance | speech-large-114K | 20k upd. | beam 1 | **gelé** (en cours) |
...
| `run_014_transformer_baseline_utterance_large_14k_v2` | transformer | utterance | speech-large-14K | early stop | greedy (v1) | freeze 5k upd. (en cours) |
...
| P1 | ST L-14k : **`run_014` v2 en cours** (Modyco) ; speechLLM L-14k **`run_012` ok** ; L-114k **`run_013` en cours** (OVH) | 1_Transformer, 2_speechLLM |
| P2 | **Gemini 3.5 Flash** — utterance mesuré (13,39) ; **relance** avec budget sortie ↑ + thinking minimal ; finir `sentence_like` | 3_Gemini |
...
- **`run_014_transformer_baseline_utterance_large_14k_v2`** : **en cours** (Modyco, juin 2026) — retry gel 5k + LR 1e-4 + early stop.
- **`run_012_speechllm_b1_utterance_large_14k`** : **terminé** (OVH, 2026-06-09) — **15,49 / 15,03** ; ~1,4 h GPU.
- **`run_013_speechllm_b1_utterance_large_114k`** : **en cours** (OVH).
- **`run_003_gemini_35_flash_utterance`** : **terminé** (local, 2026-06-09) — **11,66 / 13,39** ; 99 min ; 0,60 $ API.
- **`run_003_gemini_35_flash_sentence_like`** : **en cours** (local).
...
*Dernière mise à jour manuelle : 2026-06-09 — Gemini 3.5 utterance 13,39 (recul vs 2.5) ; run_012 speechLLM L-14k 15,03 (OVH) ; run_013/014 en cours ; sentence_like 3.5 en cours.*
