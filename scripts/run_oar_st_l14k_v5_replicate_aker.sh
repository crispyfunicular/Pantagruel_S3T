#!/usr/bin/env bash
# Job OAR IMAG — réplication ST L-14k v5 (run_043) sur H100, AMP bf16.
#
# Contexte : OAR 129663 (fp16 from scratch) → early stop @12k, BLEU test 1,11.
# Relance clean (--overwrite) avec bf16 + patience 10.
#
# Soumission depuis aker (login shell) :
#   oarsub -S -l /gpu=1,walltime=16:00:00 -p "host='lig-gpu10.imag.fr'" \
#     ~/S3T/run_oar_st_l14k_v5_replicate_aker.sh
#   # ou après rsync-code :
#   # ~/S3T/scripts/run_oar_st_l14k_v5_replicate_aker.sh

#OAR -n run_043_st_l14k_v5_replicate
set -euo pipefail
...
if [[ -z "${CFG}" ]]; then
  echo "ERROR: config bf16 introuvable (${CFG_CANDIDATES[*]})" >&2
  exit 2
fi
echo "CFG=${CFG}"
RUN_ID="run_043_aker_st_l14k_v5_replicate"
LOG="logs/run_043_st_l14k_v5_replicate_train_eval.log"
SPM_MODEL="datasets/processed/spm/fr-en_1000.model"
MANIFESTS="datasets/manifests/fr-en"
MAX_HOURS="${MAX_HOURS:-15}"
# Relance clean par défaut (éviter reprise du checkpoint BLEU~1).
FORCE_OVERWRITE="${FORCE_OVERWRITE:-1}"

echo "=== $(date -u -Iseconds) start ST replicate ${RUN_ID} (bf16) on $(hostname) ==="
...

{
  ...
  if [[ "${FORCE_OVERWRITE}" == "1" ]]; then
    ...
    echo "=== $(date -u -Iseconds) Mode --overwrite (relance clean) ==="
  elif [[ -f "runs/fr-en/${RUN_ID}/checkpoints/last.pt" ]]; then
    TRAIN_ARGS+=(--resume)
