#!/usr/bin/env bash
# Modyco — speechLLM L-14k + SpecAugment (run_039).
#
...
NOHUP_SCRIPT="${ROOT}/2_speechLLM/scripts/run_039_b1_utterance_large_14k_v5_specaug_nohup.sh"
...
echo "=== $(date -Is) Pré-vol speechLLM L-14k SpecAugment (Modyco, run_039) ==="
python 2_speechLLM/pipeline.py train \
  --config 2_speechLLM/configs/fr-en/b1_utterance_large_14k_v5_specaug.yaml \
  --run-id run_039_speechllm_b1_utterance_large_14k_v5_specaug \
  --dry-run
