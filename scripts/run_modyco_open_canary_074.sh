#!/usr/bin/env bash
# Modyco — Open baseline Canary-1B v2 (run_074 relance, piste K).
#
# OVH V100 : échec env NeMo/torch. Modyco (GPU récent) + venv dédié NeMo.
# Budget : ≤ 15 h GPU (timeout). Éval valid+test utterance (~4–10 h).
#
# NE PAS LANCER sans feu vert utilisateur.
#
# Usage (Modyco, GPU libre) :
#   cd ~/S3T && mkdir -p logs
#   nohup bash scripts/run_modyco_open_canary_074.sh \
#     > logs/run_074_modyco_launch.log 2>&1 &
#
# Via waiter (attend GPU libre) :
...
RUN_ID="run_074_modyco_open_canary_1b_utterance"
