...
REPLACEMENTS: list[tuple[re.Pattern[str], str]] = [
    ...
    (
        re.compile(r"`run_modyco_[^`]+`"),
        "`script d'orchestration (machine GPU locale)`",
    ),
    ...
    (
        re.compile(r"scripts/run_modyco_\*\.sh"),
        "scripts d'orchestration (machine GPU locale)",
    ),
    (
        re.compile(r"scripts/run_ovh_\*\.sh"),
        "scripts d'orchestration (serveur cloud GPU)",
    ),
    ...
    (
        re.compile(r"bash scripts/run_modyco_[^\s\\]+"),
        "bash scripts/<orchestration-machine-locale>",
    ),
    (
        re.compile(r"bash scripts/run_ovh_[^\s\\]+"),
        "bash scripts/<orchestration-serveur-cloud>",
    ),
    (
        re.compile(r"bash scripts/run_aker_[^\s\\]+"),
        "bash scripts/<orchestration-cluster>",
    ),
    (
        re.compile(r"nohup bash scripts/run_ovh_[^\s]+"),
        "nohup bash scripts/<orchestration-serveur-cloud>",
    ),
    (
    ...
    (
        re.compile(r"~/S3T/scripts/run_oar_[^\s']+"),
        "~/S3T/scripts/<script-oar-cluster>",
    ),
    (
    ...
    # Libellés tableaux
    (
        re.compile(r"Canary-1B\s*\(\s*machine GPU locale\s*\)", re.I),
        "Canary-1B (réplication)",
    ),
    (
        re.compile(r"SeamlessM4T v2\s*\(\s*serveur cloud GPU\s*\)", re.I),
        "SeamlessM4T v2",
    ),
