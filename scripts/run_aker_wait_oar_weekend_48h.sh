#!/usr/bin/env bash
# aker (login) — Waiter OAR week-end 48 h : run_078 encoder control → run_079 Phi-2 r4.
#
# Tour partagée IMAG : un seul job OAR bonapelm à la fois ; poll jusqu'à slot libre.
# Budget total : 48 h (deadline absolue).
#
# Usage (aker, login shell) :
#   cd ~/S3T && mkdir -p logs
#   nohup bash scripts/run_aker_wait_oar_weekend_48h.sh \
#     > logs/run_waiter_oar_weekend_48h_aker.log 2>&1 &
#   tail -f logs/run_waiter_oar_weekend_48h_aker.log
...
OAR_JOBS=(
  "run_078_speechllm_encoder_control|8:00:00|${ROOT}/scripts/run_oar_speechllm_encoder_control_aker.sh"
  "run_079_speechllm_l14k_r4|12:00:00|${ROOT}/scripts/run_oar_speechllm_l14k_r4_aker.sh"
)
...
wait_oar_slot() {
  ...
  while oarstat 2>/dev/null | grep -q bonapelm; do
    if past_deadline; then
      echo "=== $(date -Is) DEADLINE 48h — arrêt waiter OAR ==="
      exit 0
    fi
...
submit_oar() {
  ...
  local script="$3"
  chmod +x "$script"
  echo "=== $(date -Is) oarsub ${name} walltime=${walltime} ===" >&2
  local out job_id
  out="$(oarsub -l "/gpu=1,walltime=${walltime}" -n "${name}" "${script}" 2>&1)"
  echo "$out" >&2
...
wait_oar_job() {
  ...
  while true; do
    ...
    # Encore actif : Running (R) ou Waiting (W) — pas Terminated (T) / Error (E)
...
echo "=== $(date -Is) WAITER OAR week-end 48h aker — deadline $(date -d "@${DEADLINE_EPOCH}" -Is) ==="
...
for entry in "${OAR_JOBS[@]}"; do
  IFS='|' read -r name walltime script <<< "$entry"
  case "${name}" in
    run_078_speechllm_encoder_control)
      skip_if_eval "run_078_aker_speechllm_l14k_encoder_control" && continue
      wait_existing_oar "${name}" && continue
      ;;
    run_079_speechllm_l14k_r4)
      skip_if_eval "run_079_aker_speechllm_l14k" && continue
      wait_existing_oar "${name}" && continue
      ;;
  esac
  min_h="${walltime%%:*}"
...
echo "=== $(date -Is) WAITER OAR week-end 48h TERMINÉ ==="
