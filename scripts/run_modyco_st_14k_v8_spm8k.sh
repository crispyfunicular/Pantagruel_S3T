#!/usr/bin/env bash
# Modyco — ST Pantagruel-L-14k v8 SPM 8k (run_034, piste 3 Table 8, suite run_031).
#
# Recette run_026 v5 + vocab SentencePiece 8000 (aligné LeBenchmark 2.0).
# Budget : ~8–10 h GPU. À enchaîner après run_031 v7 SPM 5k.
#
# Depuis le poste local :
#   ./scripts/tour.sh ssh 'cd ~/S3T && source .venv/bin/activate && mkdir -p logs && \
#     nohup bash scripts/run_modyco_wait_st_then_st_14k_v8_spm8k.sh \
#     > logs/run_034_modyco_wait_chain.log 2>&1 &'
#
# Sur Modyco (GPU libre) :
#   cd ~/S3T && source .venv/bin/activate
#   mkdir -p logs
#   nohup bash scripts/run_modyco_st_14k_v8_spm8k.sh \
#     > logs/run_034_st_14k_v8_chain_wrapper.log 2>&1 &
#
# Forcer malgré un job GPU actif (déconseillé) :
#   bash scripts/run_modyco_st_14k_v8_spm8k.sh --force
...
ST_SCRIPT="${ROOT}/1_Transformer/scripts/run_034_baseline_utterance_large_14k_v8_spm8k_nohup.sh"
...
ensure_spm_8k() {
  local spm_model="datasets/processed/spm/fr-en_8000.model"
  if [[ -f "$spm_model" ]]; then
    echo "=== $(date -Is) SPM existant : ${spm_model} ==="
    return 0
  fi
  echo "=== $(date -Is) SPM 8k absent — entraînement avant pré-vol ==="
...
echo "=== $(date -Is) Pré-vol ST L-14k v8 SPM 8k (Modyco, run_034) ==="
python 1_Transformer/pipeline.py train \
  --config 1_Transformer/configs/fr-en/base_utterance_large_14k_v8_spm8k.yaml \
  --run-id run_034_transformer_baseline_utterance_large_14k_v8_spm8k \
  --dry-run
