#!/usr/bin/env bash
# OVH — speechLLM L-14k + Llama-3.2-3B, encodeur dégelé (run_066, piste L).
#
# Base run_052 (**16,31** test). LR 1e-5. ~6–12 h GPU.
# Prérequis : HF_TOKEN ou huggingface-cli login (Llama gated).
#
# Usage :
#   cd ~/S3T && source .venv/bin/activate && mkdir -p logs
#   nohup bash scripts/run_ovh_speechllm_066_llama_unfreeze.sh \
#     > logs/run_066_launch.log 2>&1 &
...
RUN_ID="run_066_speechllm_b2_utterance_large_14k_llama32_3b_unfreeze"
