...
Aujourd'hui, le projecteur speechLLM utilise par défaut la **dernière couche** de l'encodeur (`encoder_layer: -1` → `last_hidden_state`). Le paramètre **`model.encoder_layer`** est **implémenté** (juin 2026) ; ablations couches **9** et **6** préparées (`run_047` / `run_048`) — **non terminées** (interrompues, GPU réservé collègues).
...
Dernière mise à jour : **19 juin 2026, ~12h30** (GPU Modyco libre ; chaîne OVH terminée ; runs nuit ok).
...
**État :** GPU **libre** (19 juin ~11h — chaîne week-end **arrêtée** à la demande ; reprise prévue après ~6,5 h). `run_047` **interrompu** en cours d'entraînement.
...
| **1** | **interrompu** | `run_047` | speechLLM couche 9 | [J](#piste-j--speechllm--couche-de-sortie-de-lencodeur-pantagruel) | arrêté 19 juin — GPU réservé collègues |
| **2** | **reporté** | `run_048` | speechLLM couche 6 | [J](#piste-j--speechllm--couche-de-sortie-de-lencodeur-pantagruel) | après libération GPU (~6,5 h) |
...
| — | **Modyco** | GPU libre (collègues) ; piste J reportée | Modyco | `run_047`, `run_048` | **reporté** |
...
| **P1** | [J](#piste-j--speechllm--couche-de-sortie-de-lencodeur-pantagruel) | Ablation couches 9 puis 6 | Modyco | `run_047`, `run_048` | **reporté** (configs + code prêts) |
...
| speechLLM couche 9 | `run_047` | — | **interrompu** (piste J) |
...
| Ablation couche encodeur (Piste J) | `run_047`, `run_048` | P1 | **reporté** |
...
|-------------|-----------------|-----------|
| `run_047` (ex.) | **9** | couche « milieu-haut » souvent riche en phonétique / prosodie |
| `run_048` (ex.) | **6** | couche plus basse, plus locale |
...
**P1** — `encoder_layer` **implémenté** ; relancer [`run_modyco_wait_weekend_chain_047_048.sh`](../scripts/run_modyco_wait_weekend_chain_047_048.sh) quand le GPU Modyco est disponible.
...
| Modyco | 1–2 | `run_047` / `run_048` couche encodeur | J | **reporté** (interrompu / à relancer) |
